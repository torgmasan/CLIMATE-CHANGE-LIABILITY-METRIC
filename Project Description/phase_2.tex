...
\begin{document}
    ...
    \textbf{NIKHIL edit this first paragraph(if required) followed by your on description of gathering data like \texttt{dataset\_utilities} etc.}
    \newline
    ...
    As the application is executed, the \texttt{map\_win\_open} method in Application.Functionalities.user\_input calls the run
    ...
    country are obtained by calling the private \texttt{\_plot} method that takes in the required information and
    calls the \texttt{budget\_details} method in computation.py to return a dictionary mapping each country code to a
